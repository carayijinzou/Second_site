...
_Multiple overlaps result in higher confidence of an overlap. Get all unique sequences (duplicates don't offer more information)_
...
First, build a program that merges two fragments together: 
    ...
    for j in range(len(seq_matrix)):
      if seq_matrix[i][j]> highest[0]:
        ...
        if len(highest)>1:
          ...
          highest+[i,j]
  print(highest)
  frag_list.append(merge(frag_list.pop(highest[1]),frag_list.pop(higest[2]), highest[0]))
